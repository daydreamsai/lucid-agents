...
Lucid Agents no longer exposes adapter-local paywall factories. Configure x402 once on the agent runtime, then bind that completed runtime to Hono, Express, or TanStack. All adapters delegate invoke, stream, and task creation to the same HTTP authorization flow.
...
Bind the runtime without another payment wrapper:
...
Do not add `withPayments()`, `createHonoPaywall()`, `createExpressPaywall()`, `createTanStackPaywall()`, or `paymentMiddleware()` around Lucid routes. Those adapter-local APIs are not part of the current package surface.
