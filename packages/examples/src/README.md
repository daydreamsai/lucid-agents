...
patterns for `@lucid-agents/core` and `@lucid-agents/http`:
...
`@lucid-agents/http` offers:

- Building an agent server with `createAgentApp` (from `@lucid-agents/http`), including streaming entrypoints.
